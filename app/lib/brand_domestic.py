import pandas as pd
from transform.path import UPLOAD_DOMESTIC_FORM
...
def generate_df(brand, order_columns):
    ...
    # 개인통관고유부호
    data["원산지"] = brand["원산지"]
    data["제조사"] = brand["제조사"].str.replace(' ','')
    data["브랜드"] = brand["브랜드명"].str.replace(' ','')
    ...
    data["필수 옵션가"] = brand.apply(lambda x: option_price(
        x["색상"], x["사이즈"], x["가격"]), axis=1)
    data["옵션 재고수량"] = brand.apply(
        lambda x: option_stock_num(x["색상"], x["재고수량"]), axis=1)
...
def brand2domestic(file_path, upload_path):
    order_columns = ['상품명', '자체 상품코드', '카테고리ID', '요약설명', '판매상태', '상품상태', '판매가', '무게', '정가', '재고사용', '재고수량', 'SKU(재고번호)', '대표 이미지 파일명', '상품 상세정보', '세금', '미성년자 구매', '개인통관고유부호', '원산지', '제조사', '브랜드', '배송방법', '택배 가능 여부', '퀵서비스 가능 여부', '방문 수령 가능 여부', '배송비 결제 방법', '배송비 유형', '기본배송비', '조건부무료-상품판매가합계', '무게별 차등 배송비(기본가격)', '무게별 차등 배송비(무게)', '무게별 차등 배송비(가격)', '수량별 차등 배송비(타입)', '수량별 차등 배송비(기본 가격)', '수량별 차등 배송비(수량)', '수량별 차등 배송비(가격)', '수량별 차등 배송비(반복수량)', '수량별 차등 배송비(반복가격)', '구매금액별 차등 배송비(기본 가격)', '구매금액 차등 배송비(구매금액)', '구매금액별 차등 배송비(가격)', '묶음배송 가능', '별도설치비',
                     '지역별 배송비 사용 여부', '지역별 배송비 유형', '간편설정(제주도 추가 배송비)', '간편설정(도서산간 추가 배송비)', '우편번호 등록(시작구간)', '우편번호 등록(종료구간)', '우편번호 등록(추가배송비)', '상품구매시 적립금 지급 값', '상품구매시 적립금 지급 단위', '할인적용대상', '할인금액', '할인금액 단위', '옵션형태', '필수 옵션명', '필수 옵션값', '필수 옵션가', '옵션 재고수량', '선택 옵션명', '선택 옵션값', '선택 옵션가', '선택 옵션 재고수량', '사용자 입력형 옵션', '0원 선택옵션 최대 구매수량', '재고소진후주문가능여부', '네이버/다음 쇼핑 노출용 상품명', '네이버 쇼핑 이벤트 문구', '네이버 쇼핑 카테고리 ID', '최소 구매수량', '1회 구매시 최대 수량', '1인 최대 구매수량', '주문제작상품', '병행수입', '해외구매대행', '판매방식', '다음 쇼핑하우 노출 설정', '네이버 쇼핑 노출 설정', '네이버 페이 구매가능 설정', 'Facebook 다이내믹 광고 설정']
