...
def generate_df(brand,order_columns):
    ...
    # data["색상 개수"] = brand["색상"].apply(lambda x : len(x.split(",")))
    # data["사이즈 개수"]
    # data["재고수량 리스트"]
    ...
    # # len(x.split(",")
    # data["임시 색상"] = brand["색상"].apply(lambda x : option_split(x) if x != "" else "ONE COLOR")
    # # data["임시 사이즈"] = brand["사이즈"].apply(lambda x : option_split(x) if x != "" else "ONE SIZE")
    # # data["필수 옵션값"] = data["임시 색상"] + "\n" + data["임시 사이즈"]
    # data["필수 옵션값"] = data["임시 색상"] + "\n"
    # del data["임시 색상"]
    # # del data["임시 사이즈"]
    # data["임시 개수"] = data["필수 옵션값"].apply(lambda x : option_count(x))

    
    print(brand.head(5))
    print(data.head(5))

    # data["필수 옵션가"] = brand["가격"].astype(str) * data["임시 개수"]
    # del data["색상 개수"]
    ...
    # '네이버 쇼핑 이벤트 문구','네이버 쇼핑 카테고리 ID','최소 구매수량','1회 구매시 최대 수량','1인 최대 구매수량'
    data["주문제작상품"] = "N"
    data["병행수입"] = "N"
    data["해외구매대행"] = "N"
    data["판매방식"] = "소매"
